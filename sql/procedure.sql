...
BEGIN
  ...
  ELSEIF NOT EXISTS (
    ...
    WHERE Id_categorie_objet = p_id_categorie_objet
  ) THEN
  ...
  ELSE
    ...
    FROM OBJET
    JOIN CATEGORIE_OBJET
      ON OBJET.Id_categorie_objet = CATEGORIE_OBJET.Id_categorie_objet
    JOIN STATUT
...
BEGIN
  ...
  SELECT Id_statut
  INTO IdStatutDisponible
  FROM STATUT
  WHERE Nom_statut = 'Disponible'
  LIMIT 1;

  SELECT Id_statut
  INTO IdStatutReserve
  FROM STATUT
  WHERE Nom_statut = 'Reserve'
  LIMIT 1;

  IF NOT EXISTS (
    SELECT 1
    FROM Vue_ObjetComplet
...
BEGIN
  ...
  SELECT Id_statut
  INTO IdStatutNonDisponible
  FROM STATUT
  WHERE Nom_statut = 'Non Disponible'
  LIMIT 1;

  SELECT Id_statut
  INTO IdStatutAttente
  FROM STATUT
  WHERE Nom_statut = 'En attente validation'
  LIMIT 1;

  SELECT COUNT(*)
  INTO nb
  FROM Vue_Id_ObjetDepartement
  WHERE Id_objet = IdObjet
