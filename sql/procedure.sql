...
SELECT
    Id_utilisateur,
    Id_statut INTO v_id_proprietaire,
    v_statut
FROM
    OBJET
WHERE
...
SELECT
    'Erreur : Objet non trouvé.' AS Message;

ELSEIF v_id_proprietaire <> p_id_utilisateur THEN
SELECT
    'Erreur : cet objet ne vous appartient pas.' AS Message;

-- Vérification du statut (1 = Disponible, 3 = En attente validation)
ELSEIF v_statut NOT IN (1, 3) THEN
SELECT
    'Erreur : l objet ne peut pas être modifié (statut non autorisé).' AS Message;
...
SELECT
    'Erreur : catégorie invalide.' AS Message;

ELSE
-- Mise à jour de l'objet
UPDATE OBJET
SET
    Nom_objet = p_nom_objet,
...
WHERE
    OBJET.Id_utilisateur = p_id_utilisateur;
...
DELIMITER / / CREATE PROCEDURE filtrer_Objets_Disponibles (
...
SELECT
    ...
    Nom_utilisateur
FROM
    Vue_Objets_Disponibles
WHERE
    (
        categorie_Recherche IS NULL
        OR Nom_categorie_objet LIKE categorie_Recherche
    )
    AND (
        point_collecte_Recherche IS NULL
        OR Nom_point_de_collecte LIKE point_collecte_Recherche
    );
...
DELIMITER / / CREATE PROCEDURE Procedure_Inventaire_Utilisateur (IN Id_Utilisateur INT) BEGIN
SELECT
    Nom_objet,
    Desc_objet,
    Nom_categorie_objet,
...
FROM
    UTILISATEUR
    RIGHT JOIN OBJET ON OBJET.Id_utilisateur = UTILISATEUR.Id_utilisateur
    INNER JOIN CATEGORIE_OBJET ON CATEGORIE_OBJET.Id_categorie_objet = OBJET.Id_categorie_objet
    INNER JOIN POINT_DE_COLLECTE ON POINT_DE_COLLECTE.Id_point_collecte = OBJET.Id_point_collecte
...
DELIMITER / / CREATE PROCEDURE ReserverObjetAvecVue (IN IdUtilisateur INT, IN IdObjet INT) BEGIN DECLARE IdStatutDisponible INT;

DECLARE IdStatutReserve INT;

-- Statuts
SELECT
    Id_statut INTO IdStatutDisponible
FROM
    STATUT
WHERE
    Nom_statut = 'Disponible'
LIMIT
    1;

SELECT
    Id_statut INTO IdStatutReserve
FROM
    STATUT
WHERE
    Nom_statut = 'Reserve'
...
SELECT
    'Erreur : objet non disponible ou n’existe pas' AS Message;
...
VALUES
    (NOW (), IdUtilisateur, IdObjet);

-- Mettre à jour le statut dans la table principale
UPDATE OBJET
SET
    Id_statut = IdStatutReserve
WHERE
    Id_objet = IdObjet;

SELECT
    'Réservation effectuée avec succès. L''objet est maintenant réservé.' AS Message;
...
DELIMITER / / CREATE PROCEDURE Chef_ValiderOuRefuserObjetAvecVue (
    IN IdChef INT,
    IN IdObjet INT,
    IN Decision VARCHAR(10) -- 'Valider' ou 'Refuser'
) BEGIN DECLARE IdDepartement INT;
...
SELECT
    Id_departement INTO IdDepartement
FROM
    UTILISATEUR
WHERE
    Id_utilisateur = IdChef;
...
SELECT
    Id_statut INTO IdStatutNonDisponible
FROM
    STATUT
WHERE
    Nom_statut = 'Non Disponible'
LIMIT
    1;

SELECT
    Id_statut INTO IdStatutAttente
FROM
    STATUT
WHERE
    Nom_statut = 'En attente validation'
LIMIT
    1;
...
IF nb = 0 THEN
SELECT
    'Erreur : Objet non valide ou n''appartient pas à votre département.' AS Message;
...
UPDATE OBJET
SET
    Id_statut = IdStatutDisponible
WHERE
    Id_objet = IdObjet;

SELECT
    'Objet validé avec succès.' AS Message;

ELSEIF Decision = 'Refuser' THEN
UPDATE OBJET
SET
    Id_statut = IdStatutNonDisponible
WHERE
    Id_objet = IdObjet;

SELECT
    'Objet refusé.' AS Message;

ELSE
SELECT
    'Erreur : Décision invalide. Utilisez "Valider" ou "Refuser".' AS Message;
...
DELIMITER / / CREATE PROCEDURE CalculeStatsParDepartement (IN IdChef INT) BEGIN DECLARE IdDepartement INT;
...
SELECT
    ...
    s.Nb_reservations
FROM
    Vue_Statistiques_Departement s
WHERE
    s.Id_departement = IdDepartement
ORDER BY
    s.Nom_categorie_objet;
...
DELIMITER / / CREATE PROCEDURE Procedure_Liste_Objet_Departement (IN Id_Departement INT) BEGIN
SELECT
    Nom_objet,
    Desc_objet,
    Nom_categorie_objet,
...
FROM
    ...
    INNER JOIN STATUT ON STATUT.Id_statut = OBJET.Id_statut
WHERE
    UTILISATEUR.Id_departement = Id_Departement;
