...
DROP VIEW IF EXISTS Vue_Evenement_Passe;
...
CREATE VIEW Vue_Evenement_Passe AS
SELECT
Nom_evenement,
Localisation_evenement,
Date_evenement,
...
s.Nom_statut
FROM OBJET o
JOIN CATEGORIE_OBJET c
ON o.Id_categorie_objet = c.Id_categorie_objet
JOIN STATUT s
ON o.Id_statut = s.Id_statut;
//
...
FROM UTILISATEUR u
JOIN OBJET o
ON u.Id_utilisateur = o.Id_utilisateur
JOIN CATEGORIE_OBJET c
ON o.Id_categorie_objet = c.Id_categorie_objet
LEFT JOIN RESERVATION r
ON o.Id_objet = r.Id_objet
GROUP BY
u.Id_departement,
c.Nom_categorie_objet;
//
...
FROM OBJET AS Objet
JOIN CATEGORIE_OBJET AS Categorie
ON Objet.Id_categorie_objet = Categorie.Id_categorie_objet
JOIN STATUT AS Statut
ON Objet.Id_statut = Statut.Id_statut
JOIN UTILISATEUR AS Utilisateur
ON Objet.Id_utilisateur = Utilisateur.Id_utilisateur
JOIN POINT_DE_COLLECTE AS PointCollecte
ON Objet.Id_point_collecte = PointCollecte.Id_point_collecte;
//
