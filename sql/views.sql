...
FROM
    EVENEMENT
    INNER JOIN CATEGORIE_EVENEMENT ON CATEGORIE_EVENEMENT.Id_categorie_evenement = EVENEMENT.Id_categorie_evenement
    INNER JOIN UTILISATEUR ON UTILISATEUR.Id_utilisateur = EVENEMENT.Id_utilisateur
WHERE
    Date_evenement > NOW ();
...
FROM
    ...
    INNER JOIN STATUT ON STATUT.Id_statut = OBJET.Id_statut
    INNER JOIN UTILISATEUR ON UTILISATEUR.Id_utilisateur = OBJET.Id_utilisateur
    INNER JOIN photo ON photo.Id_objet = objet.Id_objet
WHERE
    Nom_statut = 'Disponible';
...
DELIMITER / /
CREATE VIEW
    Vue_Statistiques_Departement AS
SELECT
    u.Id_departement,
    c.Nom_categorie_objet,
